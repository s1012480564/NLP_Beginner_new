...
if __name__ == '__main__':
    ...
    parser.add_argument("--dataset_name", default="movie_reviews_val", type=str)
    ...
    labels = np.load("outputs/movie_reviews_val-val_labels.npy")
    preds = np.load(
        f"outputs/{args.method_name}-{args.model_name}-{args.dataset_name}-{args.template_name}{"-" + args.instruction if args.instruction else ""}-" + ("val" if args.method_name == "KATE" else "train") + "_preds.npy")
